...
export default class Game3D {
    ...
    toggleCamera() {
        if (this.cameraMode === 'top') {
            // Switch to third person camera (ArcRotate following player)
            ...
            // Register observer to keep camera following player
            if (!this.cameraFollowObserver) {
                this.cameraFollowObserver = this.scene.onBeforeRenderObservable.add(() => {
                    if (this.cameraMode === 'follow' && this.player) {
                        this.thirdPersonCamera.target = this.player.position;
                    }
                });
            }
